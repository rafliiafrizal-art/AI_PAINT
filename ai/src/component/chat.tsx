...
import { 
  Plus, MessageSquare, Settings, Send, 
  User, Bot, MapPin, 
  Menu, Trash2, AlertTriangle,
    ...
    try {
      ...
      if (data.success) {
        ...
        const errMsg: Message = {
          id: Date.now() + 1,
          role: 'assistant',
          content: `⚠️ ${data.message || 'Terjadi kesalahan pada server.'}`
        };
      ...
      `}>
        <div className="p-5 flex flex-col h-full min-w-70">
          ...
          <div className="mt-auto pt-4 border-t border-[#8b5a2b]/10 space-y-1">
            <button onClick={() => window.open('https://www.google.com/maps/search/toko+cat+terdekat', '_blank')} className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-[#8b5a2b]/10 text-xs font-bold transition-all">
              <MapPin size={16} /> Toko Cat Terdekat
            </button>
            <button onClick={() => setIsSettingOpen(true)} className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-[#8b5a2b]/10 text-xs font-bold transition-all">
              <Settings size={16} /> Setelan
            </button>
          </div>
      ...
      <main className={`flex-1 flex flex-col relative transition-colors duration-500 ${
        ...
        <header className={`h-16 flex items-center justify-between px-6 backdrop-blur-md sticky top-0 z-30 transition-colors ${
          ...
          <div className="flex items-center gap-4 group cursor-pointer text-right">
            ...
            <img
              src={`https://api.dicebear.com/8.x/adventurer/svg?seed=${currentUser ? currentUser.name : 'Rafli'}&backgroundColor=b07d4f`}
              alt="Profile"
              className="w-10 h-10 rounded-xl border-2 border-[#8b5a2b]/50 shadow-lg transition-all"
            />
          </div>
        </header>
              ...
              <div className="w-full max-w-2xl px-4">
                <div className={`relative flex items-end rounded-[40px] p-2 shadow-2xl transition-all duration-500 ${
                  ...
                  <textarea
                    ...
                    className={`flex-1 bg-transparent border-none focus:ring-0 focus:outline-none outline-none px-4 py-4 text-lg placeholder:text-[#8b5a2b]/20 resize-none max-h-48 custom-scrollbar ${
                      theme === 'dark' ? 'text-white' : 'text-[#4a3a2e]'
                    }`}
                  />
                  <button
                    ...
                    className="bg-[#8b5a2b] disabled:opacity-40 p-4 rounded-[22px] text-white hover:scale-105 active:scale-95 transition-all shadow-lg mb-1 mr-1 disabled:cursor-not-allowed disabled:hover:scale-100"
                  >
                    <Send size={24} />
                  </button>
                </div>
                      ...
                      <div className="flex items-start gap-3 group w-full relative">
                        <div className="flex flex-col items-center gap-1 order-first">
                          <button
                            ...
                            className="mt-2 p-2 rounded-xl transition-all opacity-0 group-hover:opacity-100 bg-[#2b2522] text-[#8b5a2b] border border-[#8b5a2b]/20 hover:scale-90 relative"
                          >
        ...
        {(messages.length > 0 || isAiLoading) && (
          <div className={`px-6 pb-10 pt-4 bg-linear-to-t z-40 transition-colors ${
            ...
            <div className="max-w-4xl mx-auto relative group">
              <div className="absolute -inset-1 bg-[#8b5a2b] rounded-[30px] blur opacity-5 group-focus-within:opacity-10 transition duration-1000" />
              <div className={`relative flex items-end rounded-[28px] p-2 pr-2 shadow-2xl transition-all duration-300 ${
                ...
                <textarea
                  ...
                  onChange={(e) => setInput(e.target.value)}
                  onKeyDown={handleKeyDown}
                  placeholder={isAiLoading ? "AI sedang menganalisis..." : "Kirim instruksi R&D lanjutan..."}
                  disabled={isAiLoading}
                  className={`flex-1 bg-transparent border-none focus:ring-0 focus:outline-none outline-none px-4 py-3 text-lg placeholder:text-[#8b5a2b]/20 resize-none max-h-48 custom-scrollbar disabled:opacity-50 ${
                    theme === 'dark' ? 'text-white' : 'text-[#4a3a2e]'
                  }`}
                />
                <button
